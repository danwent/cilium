...
var cmdMap = map[string]int{
	...
	// these are slave -> master commands only
	// treat them all as cmdNoKey
	"psave":    cmdNoKey,
	"psync":    cmdNoKey,
	"replconf": cmdNoKey,
}
...
func (rule *RedisRule) Matches(data interface{}) bool {
	...
	switch cmdType {
	case cmdInvalid:
		log.Infof("Unknown redis command '%s', rejecting command", cmd)
		return false
...
func (p *RedisParser) OnData(reply, endStream bool, dataArray [][]byte) (OpType, int) {
	...
	cmd_str := strings.ToLower(string(cmd.Args[0]))
	if cmd_str != "replconf" {
		log.Infof("about to access log cmd '%s' key '%s'", cmd_str, key)
